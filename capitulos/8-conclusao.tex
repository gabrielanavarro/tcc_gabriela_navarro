...
Após a execução dos experimentos, podemos afirmar que a utilização do algoritmo em conjunto com o \textit{RC4} é possível com um impacto pequeno no desempenho do tempo total. Algumas observações devem ser feitas, por exemplo: a utilização da fila de mensagem do \textit{Linux} impacta que deve haver uma alteração em seu tamanho para que o algoritmo funcione sem impactos e a utilização desse algoritmo é indicado para comunicações síncronas, pois somente dessa forma, os desempenhos do \textit{RC4} e do \textit{RC4} e o algoritmo se assemelham. 
...
\label{future-work}
	...
	\begin{description}
		...
		\item [Multi-\textit{threading}] Uma maneira de auxiliar o algoritmo em questão de performance, seria implementa-lo de forma a aceitar ser multi-\textit{threading} para a cifração e envio. Uma maneira de fazer com que as mensagens cheguem na ordem deve ser pensada.
	\end{description}
